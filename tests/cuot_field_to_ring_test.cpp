// cuOT-backed field→ring conversion test (Task M3-T5).
//
// cuOT counterpart of tests/field_to_ring_or_aux_test.cpp (the CPU EmpOTProvider
// baseline). Runs the EXACT SAME boundary block and 100000-sample random cases
// through FieldToRing::or_aux, but backed by CuotProvider (GPU Ferret) instead
// of EmpOTProvider. Acceptance #1 of M3-T5 — "CPU backend and cuOT backend
// both pass" — is exercised on identical inputs.
//
//   (out_A[i] + out_B[i]) & mask == ((msb_A[i] | msb_B[i]) * p) & mask
//   bw_y=32, p=4293918721.
//
...
// exposed here only for the test, like cuot_ring_to_field_test.cpp).
//
...
// send_cot/recv_cot layer MITCCRH hashing on that block-RCOT (M2-T3 adapter,
// logic PASS), so the OR_AUX reconstruction inherits the same ~2% floor. This
// test reports the actual bad count / rate and is an ADAPTER-LOGIC / WIRING
// pass if the only failures are the inherited floor (rate ≤ 4%); above that
// the FieldToRing primitive or cuOT wiring is buggy. Per user direction
// (2026-06-28), this known floor is accepted and documented; M3-T5's strict
// "random stress pass" is NOT met cleanly by the cuOT backend — only the CPU
// backend meets it. See docs/research/m3-cuot-integration.md.
//
// Mirrors field_to_ring_or_aux_test.cpp's cases verbatim (same seeds, same
...
//   sudo ./run_cuot_field_to_ring_test.sh [PORT]
...
int run_sender(int port, int gpu) {
    ...
    std::cout << "[sender] cuOT field_to_ring OR_AUX sent: boundary=" << nb
              << " random=" << n << " p=" << p << " bw=" << bw_y
              << " gpu=" << gpu << std::endl;
    return 0;
}
...
int run_receiver(int port, int gpu) {
    ...
    std::cout << "[receiver] cuOT field_to_ring OR_AUX: boundary_bad="
              << boundary_bad << " random_bad=" << random_bad << "/" << n
              << " (" << rate << "%)  p=" << p << " bw=" << bw_y
              << " gpu=" << gpu << std::endl;

    if (rate > kAdapterLogicMaxRate) {
        std::cerr << "FAIL cuOT field_to_ring random rate " << rate
                  << "% > " << kAdapterLogicMaxRate
                  << "% (FieldToRing/cuOT wiring buggy, not RCOT floor)\n";
        ++fails;
    }
    if (fails != 0) {
        std::cerr << "cuOT field_to_ring OR_AUX test FAILED (wiring)\n";
        return 1;
    }
    std::cout << "cuOT field_to_ring OR_AUX test PASSED (wiring + adapter "
...
int main(int argc, char** argv) {
    if (argc < 3) {
        std::cout << "Usage: cuot_field_to_ring_test <party 1|2> <port> [gpu]\n"
