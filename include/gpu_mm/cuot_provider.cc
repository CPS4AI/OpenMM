...
CuotProvider::CuotProvider(OTParty role, int port, int gpu, const char* address,
    ...
    try {
        ...
        // (the default/only tuned preset). pre_file="" -> uses the standard
        // ./data/pre_ot_data_reg_{send,recv} cache path from constants.h.
...
// cuOT provides NO arithmetic COT natively (GF(2^128) only). M2-T3 adapter:
// layer MITCCRH hashing on the block chosen-COT (send_cot_blocks/recv_cot_blocks)
// to produce uint64_t arithmetic COT, mirroring SCI SilentOT::send_ot_cam_cc /
