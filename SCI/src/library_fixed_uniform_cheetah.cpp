...
void Ring2Field(int32_t size, intType *inArr, intType *outputArr) {
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        auto funcRing2FieldThread = [] (int tid, int32_t size,
                                        intType *inpArr, intType *outpArr,
                                        const int32_t ring_bw, const uint64_t field_mod)  {
            truncationArr[tid]->ring_to_field(size, inpArr, outpArr, ring_bw, field_mod);
        };

        truncThreads[i] = std::thread(funcRing2FieldThread, i, curSize, inArr + offset, outputArr + offset, bitlength, field_mod);
    }
    for (int i = 0; i < num_threads; ++i) {
        truncThreads[i].join();
    }
...
void Field2Ring(int32_t size, intType *inArr, intType *outputArr) {
    std::thread truncThreads[num_threads];
    int chunk_size = (size / (8 * num_threads)) * 8;

    auto funcField2RingThread = [] (int tid, int32_t size,
                                    intType *inpArr, intType *outpArr,
                                    const uint64_t field_mod, const int32_t ring_bw)  {
        truncationArr[tid]->field_to_ring(size, inpArr, outpArr, field_mod, ring_bw);
    };
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        truncThreads[i] = std::thread(funcField2RingThread, i, curSize, inArr + offset, outputArr + offset, field_mod, bitlength);
    }
    for (int i = 0; i < num_threads; ++i) {
        truncThreads[i].join();
    }
...
void Field2Ring_with_Truncate(int32_t size, intType *inArr, intType *outputArr) {
    std::thread truncThreads[num_threads];
    int chunk_size = (size / (8 * num_threads)) * 8;

    auto funcField2RingThread = [] (int tid, int32_t size,
                                    intType *inpArr, intType *outpArr,
                                    const uint64_t field_mod, const int32_t ring_bw, const int32_t shift_bw)  {
        truncationArr[tid]->field_to_ring_with_truncate(size, inpArr, outpArr, field_mod, ring_bw, shift_bw);
    };
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        truncThreads[i] = std::thread(funcField2RingThread, i, curSize, inArr + offset, outputArr + offset, field_mod, bitlength, kScale);
    }
    for (int i = 0; i < num_threads; ++i) {
        truncThreads[i].join();
    }
...
void ElemWiseActModelVectorMult(int32_t size, intType *inArr,
  ...
  gemini::CheetahLinear::BNMeta meta;
  meta.target_base_mod = field_mod;
  meta.is_shared_input = kIsSharedInput;
  meta.vec_shape = gemini::TensorShape({size});

  gemini::Tensor<intType> field_in_vct, field_out_vct;
  gemini::Tensor<intType> field_scale_vec;

  field_in_vct.Reshape(meta.vec_shape);
  if (meta.is_shared_input) {
      Ring2Field(size, inArr, field_in_vct.data());
  } else {
      std::transform(inArr, inArr + size, field_in_vct.data(), getFieldElt);
  }

  field_scale_vec.Reshape(meta.vec_shape);
  if (cheetah_linear->party() == SERVER) {
      std::transform(multArrVec, multArrVec + size, field_scale_vec.data(), getFieldElt);
  }

  cheetah_linear->bn(field_in_vct, field_scale_vec, meta, field_out_vct); // element-wise product BatchNorm

//  Field2Ring(size, field_out_vct.data(), outputArr);
  Field2Ring_with_Truncate(size, field_out_vct.data(), outputArr);

  if (cheetah_linear->party() == SERVER) {
    cheetah_linear->safe_erase(field_scale_vec.data(), field_scale_vec.NumElements());
  }
  ...
  if (party == SERVER) {
    ...
    bool pass = true;
    for (int i = 0; i < size; i++) {
      int64_t gnd = getSignedVal(VoutputVec[i]) / (1 << kScale);
      int64_t cmp = VoutputArr[i];
      if (std::abs(gnd - cmp) > 1) {
        if (pass) {
