...
class SilentOT : public sci::OT<SilentOT<IO>> {
 public:
  ...
  void send_cot_prime(uint64_t* data0, const uint64_t* corr, int length, uint64_t p) {
    send_ot_cam_cc_prime(data0, corr, length, p);
  }

  void recv_cot_prime(uint64_t* data, const bool* b, int length, uint64_t p) {
    recv_ot_cam_cc_prime(data, b, length, p);
  }
  ...
  void send_ot_cam_cc_prime(uint64_t* data0, const uint64_t* corr, int64_t length, uint64_t p) {
    block * rcm_data = new block[length];
    send_ot_rcm_cc(rcm_data, length);
    ...
    const int l = ceil(log2(p) / 8.0);
    block pad[2 * ot_bsize];
    uint64_t corr_data[ot_bsize];
    uint8_t * send_data_buff = new uint8_t[ot_bsize * l];
    ...
    for (int64_t i = 0; i < length; i += ot_bsize) {
      ...
      for (int j = i; j < i + ot_bsize and j < length; ++j) {
        __uint128_t tmp0 = (*(__uint128_t *) &pad[2 * (j - i)]) % p;
        __uint128_t tmp1 = (*(__uint128_t *) &pad[2 * (j - i) + 1]) % p;
        data0[j] = static_cast<uint64_t>(tmp0);
        corr_data[j - i] = static_cast<uint64_t>((tmp0 + tmp1 + corr[j]) % p);
        std::memcpy(&send_data_buff[(j - i) * l], &corr_data[j - i], l);
      }
      ferret->io->send_data(send_data_buff, std::min(ot_bsize, length - i) * l);
    }
    delete[] rcm_data;
    delete[] send_data_buff;
  }
  ...
  void recv_ot_cam_cc_prime(uint64_t* data, const bool* b, int64_t length, uint64_t p) {
    block *rcm_data = new block[length];
    recv_ot_rcm_cc(rcm_data, b, length);
    block s;
    ferret->io->recv_block(&s, 1);
...
//    ferret->io->flush();
    ...
    const int l = ceil(log2(p) / 8.0);
    block pad[ot_bsize];
    uint64_t *corr_data = new uint64_t[ot_bsize];
    uint8_t *recv_data_buff = new uint8_t[ot_bsize * l];

    for (int64_t i = 0; i < length; i += ot_bsize) {
      memcpy(pad, rcm_data + i, std::min(ot_bsize, length - i) * sizeof(block));
      ferret->mitccrh.template hash<ot_bsize, 1>(pad);

      ferret->io->recv_data(recv_data_buff, std::min(ot_bsize, length - i) * l);
      for (int j = i; j < i + ot_bsize and j < length; ++j) {
        uint64_t tmp = 0;
        std::memcpy(&tmp, &recv_data_buff[(j - i) * l], l);
        corr_data[j - i] = tmp;
      }

      for (int j = i; j < i + ot_bsize and j < length; ++j) {
        __uint128_t tmp = (*(__uint128_t *) &pad[j - i]) % p;
        if (b[j])
          data[j] = static_cast<uint64_t>((corr_data[j - i] + (p - tmp)) % p);
        else
          data[j] = static_cast<uint64_t>(tmp);
      }
    }
    delete[] rcm_data;
  }
