...
class CheetahLinear {
 public:
  ...
  // (x0 + x1) * (y0 + y1)
  // Use element-wise multiplication calculate cross terms x0 * y1 + x1 * y0
  void BOLE(const Tensor<uint64_t> & input_x_vector,
 ...
 private:
  ...
  std::shared_ptr<seal::SEALContext> simd_bn_context_;
  std::shared_ptr<seal::SecretKey> simd_bn_sk_;  // Bob only
  std::shared_ptr<seal::PublicKey> simd_bn_pk_;  // Alice only
